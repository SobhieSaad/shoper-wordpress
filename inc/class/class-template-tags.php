...
class shoper_Post_Meta {
	...
	public function __construct() {
		...
		add_action( 'shoper_do_tags', array( $this, 'render_tags_list' ) );
		add_action( 'shoper_site_content_type', array( $this, 'render_meta_list' ), 20, 1 );
		add_action( 'shoper_meta_info', array( $this, 'render_meta_list' ), 10, 2 );
	...
	public function render_meta_list( $order, $wrp_class = '' ) {
		...
		foreach ( $order as $meta ) {
			switch ( $meta ) {
				case 'author':
					...
					$markup .= ' <span>'. esc_html__( 'By - ','shoper' ) .'</span>';
				...
				case 'date':
					...
					$markup .= esc_html__( 'Posted on ','shoper' );
					$markup .= $this->get_time_tags();
					$markup .= '</li>';
					break;
				case 'category':
					$markup .= '<li class="meta category">';
					$markup .= esc_html__( 'Posted in ','shoper' );
		...
		echo wp_kses( $markup, shoper_alowed_tags() );
	}
	...
	private function get_comments() {
		...
		if ( $comments_number == 0 ) {
			...
			$comments = sprintf( _n( '%s Comment', '%s Comments', $comments_number, 'shoper' ), $comments_number );
		}
	...
	public function render_tags_list() {
		...
		$html .= '<span>' . esc_html__( 'Tags', 'shoper' ) . ':</span>';
		foreach ( $tags as $tag ) {
